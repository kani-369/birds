...
# 🔹 1. Setup save directory (Assuming Drive is mounted externally in Colab)
if os.path.exists('/content/drive/MyDrive'):
    BASE_SAVE_DIR = "/content/drive/MyDrive/birdclef_models"
    LOCAL_TMP_DIR = "/content"
else:
    BASE_SAVE_DIR = "./birdclef_models"
    LOCAL_TMP_DIR = "."
...
# 🔹 2. Add FULL config + environment capture
config = {
    "learning_rate": 0.001,
    "batch_size": 8,
    "num_epochs": 10,
...
def main():
    ...
    # If in Colab, the unzipped dataset sits at /content/
    if os.path.exists('/content/train_audio'):
        if os.path.exists('/content/train_metadata.csv'):
            csv_path = "/content/train_metadata.csv" 
        else:
            csv_path = "/content/train.csv"
        audio_dir = "/content/train_audio"
    else:
        csv_path = "./train.csv"
        audio_dir = "./train_audio"
    
    if not os.path.exists(csv_path):
        print(f"Error: {csv_path} not found. Please ensure your dataset is placed here.")
        return
